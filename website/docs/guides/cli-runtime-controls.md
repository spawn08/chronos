...
`--stream`/`--no-stream`/`-s` are process-wide flags, like `--debug`/`--trace`: they work anywhere on the command line, not just immediately after `run`/`team run` — `chronos --stream run "..."` and `chronos run "..." --stream` are equivalent. `CHRONOS_STREAM=true|false` is the environment equivalent.

## Structured (JSON) output

`--output-schema <file>` points at a JSON Schema file and requests JSON conforming to it, overriding any `output_schema:` already set in the agent's YAML config. Like `--stream`, it is a process-wide flag and works anywhere on the command line:
