...
func (s *Store) ListEventsPaged(ctx context.Context, sessionID string, afterSeq int64, limit int, cursor string) (*storage.EventPage, error) {
	...
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, tenant_id, session_id, seq_num, type, payload, created_at FROM events WHERE tenant_id=? AND session_id=? AND seq_num>? ORDER BY seq_num LIMIT ?`,
...
func (s *Store) ListCheckpointsPaged(ctx context.Context, sessionID string, limit int, cursor string) (*storage.CheckpointPage, error) {
	...
	tenant := storage.TenantFromContext(ctx)
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, tenant_id, session_id, run_id, node_id, state, seq_num, created_at FROM checkpoints WHERE tenant_id=? AND session_id=? AND seq_num>? ORDER BY seq_num LIMIT ?`,
...
func (s *Store) ListTracesPaged(ctx context.Context, sessionID string, limit int, cursor string) (*storage.TracePage, error) {
	...
	if afterID == "" {
		r, err = s.db.QueryContext(ctx,
			`SELECT `+cols+` FROM traces WHERE tenant_id=? AND session_id=? ORDER BY id LIMIT ?`,
			tenant, sessionID, limit+1,
		)
	} else {
		r, err = s.db.QueryContext(ctx,
			`SELECT `+cols+` FROM traces WHERE tenant_id=? AND session_id=? AND id>? ORDER BY id LIMIT ?`,
...
func (s *Store) TrimCheckpoints(ctx context.Context, sessionID string, keep int) (int64, error) {
	...
	res, err := s.db.ExecContext(ctx,
		`DELETE FROM checkpoints WHERE tenant_id=? AND session_id=? AND seq_num NOT IN (
			SELECT seq_num FROM checkpoints WHERE tenant_id=? AND session_id=? ORDER BY seq_num DESC LIMIT ?
		)`,
		tenant, sessionID, tenant, sessionID, keep,
	)
	if err != nil {
...
func (s *Store) AppendEvents(ctx context.Context, events []*storage.Event) error {
	...
	for start := 0; start < len(events); start += rowsPerChunk {
		...
		b.WriteString(`INSERT OR IGNORE INTO events (id, tenant_id, session_id, seq_num, type, payload, created_at) VALUES `)
		...
		for i, e := range chunk {
			...
			args = append(args, e.ID, tenant, e.SessionID, e.SeqNum, e.Type, string(payload), e.CreatedAt)
		}
...
func (s *Store) InsertTraces(ctx context.Context, traces []*storage.Trace) error {
	...
	for start := 0; start < len(traces); start += rowsPerChunk {
		...
		var b strings.Builder
		b.WriteString(`INSERT OR REPLACE INTO traces (id, tenant_id, session_id, parent_id, name, kind, input, output, error, started_at, ended_at) VALUES `)
		args := make([]any, 0, len(chunk)*paramsPerRow)
		for i, t := range chunk {
			if i > 0 {
				b.WriteByte(',')
			...
			args = append(args, t.ID, tenant, t.SessionID, t.ParentID, t.Name, t.Kind, string(inp), string(outp), t.Error, t.StartedAt, t.EndedAt)
		}
		if _, err := tx.ExecContext(ctx, b.String(), args...); err != nil {
